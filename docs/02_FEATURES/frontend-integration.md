...
- 파일 선택·드래그 앤 드롭 모두 받고, `fileError()` 가 **백엔드와 같은 규칙**(`.pdf`·`.hwp`·`.hwpx`·`.txt`·`.md`,
  파일당 100MB, 빈 파일 거부)으로 먼저 걸러 올리지 않는다. 걸러진 파일도 사유와 함께 목록에 남긴다.
  크기 상한은 백엔드 `MAX_ATTACHMENT_MB` 와 **같은 값을 손으로 맞춘다**(`MAX_FILE_MB`) — 정적 프론트라
  서버 설정을 읽어올 방법이 없다. 어긋나면 프론트가 통과시킨 파일을 서버가 `FILE_TOO_LARGE` 로 거절하며,
  그 경우에도 업로드 응답의 `rejected` 로 사용자에게 그대로 보인다(조용히 사라지지는 않는다).
- 분량 상한(`MAX_ATTACHMENT_CHUNKS`)에 걸린 첨부는 `truncated: true` 인 `ready` 로 오고, 파일 행과 완료
  화면에 "앞부분만 색인" 을 표시한다. 실패로 보이면 안 되고(질의에는 쓰인다), 감춰도 안 된다(답이 비는
  이유가 된다).
