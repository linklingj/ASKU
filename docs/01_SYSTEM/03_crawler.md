...
처음 보는 학교는 크롤 시작 시 규격을 확보한다. 목록 1페이지와 상세 몇 건을 표본으로 받아 아래 순서로 처리한다.

1. **알려진 게시판 제품의 템플릿을 대본다**(`app/spec_templates.py`). 국내 대학은 같은 제품을 널리 써, 실측한 학교 7곳 중 6곳이 이 단계에서 끝났다(LLM 호출 0회). 같은 제품이라도 학교마다 마크업이 갈리므로 행·필드 선택자를 후보 목록으로 두고 **값이 나오는 첫 후보**를 쓴다 — 건국대는 목록을 표로, 연세대는 `ul > li`로 그린다.
2. 맞는 템플릿이 없으면 LLM으로 규격을 만든다([§7-2](#7-2-규격-자동-생성)). **기본으로 켜지 않는다**(`SPEC_AUTOGEN`). 호출당 약 4만 토큰이 들고, 표본 몇 건으로 채점하는 검증은 깊은 페이지의 어긋남까지 보증하지 못한다.
3. 어느 쪽도 안 되면 규격 없이 진행한다. 공용 파서로 수집되며, 품질은 §6-2 지표에 남는다.

템플릿과 자동 생성 규격은 **같은 검증기**로 판정한다. 기준이 다르면 어느 쪽이 나은지 비교할 수 없고, 템플릿만 느슨하게 통과하는 일이 생긴다.
...
알려진 템플릿과 맞지 않는 학교는 게시판 HTML을 LLM에 보여 주고 규격을 받는다(`app/spec_generator.py`). 학교 등록 시 한 번만 부르며, 통과한 규격으로 수집할 때는 LLM을 쓰지 않는다.

**받은 규격을 그대로 믿지 않는다.** 실제 페이지를 파싱해 §6-2 검증기로 채점하고, 실패하면 무엇이 왜 틀렸는지를 붙여 다시 시킨다. 같은 프롬프트로 재시도하면 같은 실수를 반복한다 — 아주대 실측에서 지적이 3건 → 1건으로 줄며 제목 선택자가 교정됐다.

채택 기준은 지적의 무게에 따라 나눈다.

| 등급 | 예 | 처리 |
|---|---|---|
| 차단 | 목록 0행, 제목 없음 | 규격을 쓸 수 없다 |
| 표본 | 본문 없음, 제목 불일치 | 표본 절반까지 허용하고 경고로 남긴다 |
| 경고 | 날짜 미달, 페이지네이션 이상 | 통과시키되 알린다 |

전부 똑같이 막으면 날짜를 주지 않는 게시판 하나 때문에 학교를 통째로 버린다. 반대로 표본 지적을 모두 무시하면 본문이 비는 규격이 통과한다. 본문이 이미지 한 장뿐인 공지가 실제로 있어(아주대 학위수여일 안내) 어떤 선택자로도 통과할 수 없다.

제공자 오류(429·503)는 규격 품질과 무관하므로 지적으로 돌려주지 않고, 지수 백오프로 재시도한다. 규격 시도 횟수와 따로 세지 않으면 서버가 바쁜 동안 멀쩡한 학교를 포기하게 된다.

개발 중에는 명령으로 직접 확인한다.

```bash
PYTHONPATH=backend python3 backend/scripts/generate_spec.py <목록 URL>          # 출력만
PYTHONPATH=backend python3 backend/scripts/generate_spec.py <목록 URL> --save   # 등록까지
```
