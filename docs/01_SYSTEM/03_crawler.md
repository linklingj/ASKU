...
크롤이 끝나면 §6-2 검증 결과를 보고, **차단급 지적**(목록 0행·제목 없음 등)이 있으면 규격을 다시 만든다. 학교가 홈페이지를 개편하면 선택자가 어긋나 수집이 0건이 되는데, 크롤은 정상 종료하므로 지표를 보지 않으면 아무도 알아채지 못한다. 학교가 늘수록 사람이 매번 손보기 어렵다.

- 경고급 지적(날짜 미달 등)으로는 재생성하지 않는다. 멀쩡한 규격을 갈아엎게 된다.
- **사람이 쓴 규격(`source: "human"`)은 덮어쓰지 않는다.** 손으로 고친 결정을 자동 생성이 밀어내면 왜 바뀌었는지 알 수 없다. 경고만 남기고 사람이 판단한다.
- 재생성도 템플릿을 먼저 대본다. 개편으로 알려진 제품으로 갈아탄 경우가 있어, 대개 LLM 없이 복구된다.
- 새 규격도 검증을 통과해야 저장된다. 통과하지 못하면 기존 규격을 그대로 둔다.
