...
| 역할 | 폰트 | 비고 |
|---|---|---|
| 로고 | **SVG 애셋** (`frontend/src/assets/`) | 워드마크 `logo_full.svg`, 글자별 `logo_{A,S,K,U}.svg`(파비콘은 `logo_A.svg` 재사용) |
| 디스플레이 | **Hostero** (`uploads/HosteroRegular-…ttf`) | 숫자·카운터(01·02…). 워드마크는 위 SVG 사용 |
| 헤딩 | **Archivo** 800 | 섹션 제목, 타이트 자간 `-0.02em` |
| 본문 | **Archivo** 400/600 | 15–20px, line-height 1.5–1.6 |

- 반응형 크기는 `clamp()` 로. 헤딩은 문장 단위로 줄바꿈(`.line` 블록).
- **로고 SVG 규칙**: 애셋은 배경판 없는 투명 SVG(잉크 `#16181c`)라 어느 배경에도 올릴 수 있다.
  어두운 배경 위에서는 `filter:invert(1)`(`.logo-ink`)로 뒤집어 쓴다. 파비콘도 같은 투명 `logo_A.svg`.
  글자별 파일은 밑선이 맞춰진 채로 잘려 있어,
  **높이를 원본 viewBox 높이 비율대로** 주고 아래쪽 정렬하면 원본 배열이 그대로 재현된다.
...
| hover(밝은 버튼) | ~0.22s | `.btn-ink` — 배경이 잉크색으로 차오르고 글씨는 배경색으로 반전 |
| hover(붉은 버튼) | ~0.32s | `.btn-accent` — 흐르는 그라데이션이 서서히 덮이고 글씨가 굵어짐(800→900) |

- **버튼 hover 는 위 두 클래스로만** 구현한다(정의는 공유 `_ds/…/styles.css`). 페이지마다 따로 hover 를
  선언하지 않는다 — 인라인 `style` 로 배경을 준 버튼이 많아 각자 구현하면 특이도 싸움에서 밀려
  "글씨만 배경색으로 바뀌어 안 보이는" 버그가 난다(그래서 공통 클래스는 hover 선언만 `!important`).
...
이미 [`frontend/src/index.html`](../../frontend/src/index.html) 에 구현. 섹션: 히어로(아래 인트로 타임라인) → 사용 흐름 → Why(마우스 반응 그라디언트) → 지원 학교(로고 마퀴·슬롯머신 카운터) → 만든 사람들 → 푸터. 두 CTA가 `/find`·`/register` 로 연결된다. 새 페이지는 이 파일의 토큰·모션을 **재사용**하고 재구현하지 않는다.

**히어로 인트로 타임라인 (`#top`)**

| 시점 | 등장 |
|---|---|
| 0 – 2.3s | 로고 글자 SVG 4개가 0.3s 간격으로, 1.25s 동안 아래에서 튀어 오른다(overshoot 이징). 이 동안 **내비까지 전부 숨김** |
| 2.4s – | `우리 학교 정보, / 그만 찾고 물어보세요.` 를 **글자 단위로 왼쪽부터** 0.05s 간격으로 올린다(JS 가 문장을 `.rise` 글자로 쪼갬) |
| 문장 종료 +0.2s | 설명 문단이 한 번에 |
| 문장 종료 +0.55s | CTA 버튼 |

- 순서는 전부 `transition-delay` 로만 만든다(타이머 체인 없음) — 스크롤로 되감았다 와도 그대로 재생된다.
- 주의 두 가지: (1) 문장 한 줄 안에 `.rise` 가 여러 개라 리빌 옵저버는 부모마다 **대상 목록**을 들고 있어야 한다(하나만 기억하면 마지막 글자만 뜬다).
  (2) `.hero-logo .rise` 는 `.rise.vis` 와 특이도가 같아 **도착 상태(`.hero-logo .rise.vis`)도 같이 적어야** 한다.

**사용 흐름 섹션(`#flow`) — 스크롤 고정 + 순차 등장**

- 어두운 배경(`#201e1d`) 위 **2열 × 6행 그리드** — `등록 흐름 · Register` / `질의 흐름 · Ask`.
  좌우 단계가 같은 행을 공유하고 `align-self:start` 라, 아이콘 높이가 달라도 라벨·화살표 줄이 서로 맞는다.
- 각 흐름의 단계는 좌·우로 번갈아 밀어(`translate` 속성, 진폭 `--sx`) **S 자 흐름**을 그리고, 화살표도 진행 방향으로
  휜 곡선 아이콘을 쓴다(되돌아오는 구간은 `scale:-1 1` 로 반전). 등장 애니메이션이 `transform` 을 쓰므로 배치는
  `translate`/`scale` 개별 속성으로 준다 — 서로 덮어쓰지 않는다.
- **스크롤을 가로채지 않는다.** 섹션 높이를 `100vh + 요소수 × 34vh` 로 늘리고 내부를 `position:sticky; top:0` 으로 붙인 뒤,
  스크롤 진행도(0–1)를 `data-seq` 순서에 매핑해 하나씩 `.on` 을 켠다. 마지막 요소까지 켜지면 섹션이 자연히 풀려 아래로 이어진다.
  (`preventDefault` 로 스크롤을 잡는 방식은 키보드·트랙패드·접근성을 깨뜨려 쓰지 않는다.)
- 조상에 `overflow-x:hidden` 이 있으면 sticky 가 죽는다 → 랜딩 루트는 **`overflow-x:clip`**.
- 예시 데모는 처음 등장할 때 시작해 **무한 반복**한다: URL·질문은 목록에서 무작위로 골라 타이핑→유지→삭제,
  학교명은 슬롯머신으로 굴리다 감속해 멈추고 2초 뒤 다시 돈다. 문구는 마크업의 `data-texts`(`|` 구분)·`data-pool` 로만 바꾼다.
- 아이콘은 `assets/*.gif`. 원본(흰 배경·검정 선화·청록 포인트)을 **어두운 배경용으로 리컬러**해 둔 상태 —
  포인트색 → accent, 선화 → 크림, 흰 배경 → 투명, 256px.
  QA 아이콘은 원본(28.4s)이 너무 느려 **프레임을 절반만 남기고 딜레이도 줄여 한 바퀴 3.25s** 로 다시 저장했다.
...
- 목록은 항상 **이름 가나다순**(`localeCompare(…, "ko")`)으로 정렬해 보여준다. 백엔드 응답 순서에 의존하지 않는다.
...
- 그래프 우상단(상단 버튼 줄 아래)에 **독립 패널**로 둔다. 왼쪽 사이드바는 **노드 설명 전용**이라 필터를 섞지 않는다.
- 헤더를 누르면 접기/펼치기(`state.filterOpen`), 헤더에 `보이는 타입/전체` 카운트를 표시한다.
- **전체 보기 / 전체 숨기기** 버튼과 타입별 토글 행(색 스와치·이름·개수). 타입이 없으면 패널 자체를 그리지 않는다.

**노드 선택 → 정보 패널**

- 왼쪽 사이드바에 엔티티 상세: `type`, `name`, `attributes`(마감일·금액 등), **근거 문서 링크**(`sources`), 이웃 관계 목록.
...
- **질의 중 동작**: 입력창을 `disabled` 로 잠그고 플레이스홀더를 비운다(중복 질문·편집 방지).
  질문 텍스트는 입력창에서 사라져 **답변 카드 상단("질문" 블록)** 으로 옮겨가고, 카드는 즉시 열려
  본문에 `…` 점 애니메이션으로 대기를 표시한다(사용자 모델 경로면 진행 단계 문구를 함께). 답변이
  끝나면 잠금을 풀고 입력창에 포커스를 되돌린다.
- 예시 질문 칩은 **첫 진입에만** 노출하고, 한 번 질문한 뒤에는 감춘다.
...
- 그 아래 **문서 첨부(선택)** — 수강편람 PDF·학칙 HWP 처럼 크롤링으로 닿지 않는 문서를 함께 올린다
  (`.pdf`·`.hwp`·`.hwpx`·`.txt`·`.md`, 파일당 100MB. [`01_backend-api.md`](01_backend-api.md) §2.4-1).
  지원하지 않는 파일은 올리기 전에 사유와 함께 목록에서 걸러낸다. 형식·크기 안내 문구는
  검증 상수에서 만들어, 상한만 바꿨을 때 문구가 어긋나지 않게 한다.
...
- `truncated: true` 인 첨부는 `ready` 지만 분량 상한에 걸려 앞부분만 들어간 것이라, 파일 행과 완료 화면에
  모두 표시한다 — 문서 뒷부분이 답에 안 나오는 이유를 알 수 있어야 한다.
