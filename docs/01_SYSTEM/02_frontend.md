...
이미 [`frontend/src/index.html`](../../frontend/src/index.html) 에 구현. 섹션: 히어로(로고 글자 SVG 4개를 `.rise` 로 순차 등장) → 사용 흐름(등록/질의 스텝) → Why(마우스 반응 그라디언트) → 지원 학교(로고 마퀴) → 만든 사람들 → 푸터. 두 CTA가 `/find`·`/register` 로 연결된다. 새 페이지는 이 파일의 토큰·모션을 **재사용**하고 재구현하지 않는다.
